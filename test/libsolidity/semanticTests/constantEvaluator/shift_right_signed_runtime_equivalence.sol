...
contract C {
    ...
    int256 constant LITERAL_WRAP = -2**255 >> U_256; // = -1
    uint[LITERAL_WRAP * -1] a;
    int8 constant CONST_NO_WRAP = I8_NEGATIVE_63 >> 1; // = -32
    uint[CONST_NO_WRAP * -1] b;
    int8 constant CONST_WRAP = I8_POSITIVE_127 >> 8; // = 0
    uint[CONST_WRAP + 1] c;
    int8 constant CONST_WRAP_SIGN_EXT = I8_NEGATIVE_128 >> 8; // = -1
    uint[CONST_WRAP_SIGN_EXT * -1] d;
    ...
    function testConstWrapSignExtendEquivalence() public view returns (bool) {
        ...
        return
            CONST_WRAP_SIGN_EXT == runTimeResult &&
            d.length == 1;
    }
}
// ----
